...
VOLATILE -- Function modifies the database, so it can't be STABLE
AS $$
DECLARE
...
-- Create a view to expose the function via PostgREST
DROP VIEW IF EXISTS sync_games_view;
CREATE VIEW sync_games_view AS
SELECT sync_games() as result;

-- Grant access to the view
GRANT SELECT ON sync_games_view TO authenticated;
GRANT SELECT ON sync_games_view TO anon;

-- Ensure RLS is enabled on the view with proper security context
ALTER VIEW sync_games_view OWNER TO authenticated;