...
CREATE OR REPLACE FUNCTION sync_games(payload json DEFAULT '{}'::json)
RETURNS json
LANGUAGE plpgsql
SECURITY DEFINER
...
GRANT EXECUTE ON FUNCTION sync_games(json) TO authenticated;
GRANT EXECUTE ON FUNCTION sync_games(json) TO anon;

-- Comment to help PostgREST recognize the function
COMMENT ON FUNCTION sync_games(json) IS 'Syncs upcoming games for the next 14 days';