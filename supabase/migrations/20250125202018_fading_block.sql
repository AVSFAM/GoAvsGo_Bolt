...
DROP VIEW IF EXISTS sync_games_rpc;
DROP FUNCTION IF EXISTS sync_games();

-- Create sync_games function with proper PostgREST support
CREATE OR REPLACE FUNCTION sync_games(dummy_param jsonb DEFAULT '{}'::jsonb)
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
...
GRANT EXECUTE ON FUNCTION sync_games(jsonb) TO authenticated;
GRANT EXECUTE ON FUNCTION sync_games(jsonb) TO anon;

-- Add comment to help PostgREST
COMMENT ON FUNCTION sync_games(jsonb) IS 'Syncs upcoming games for the next 14 days';